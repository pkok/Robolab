...
To tackle the problem of false positives, \emph{a contrario} validation is employed to reject or validate candidate ellipses. The \emph{a contrario} theory employs the non-accidentalness principle (which says that ``we see nothing in noise''). Therefore, candidate ellipses that have a higher chance to be observed in noise are discarded. % how is that done? 
...
%maybe put this in the appendix.

%\begin{algorithm}[H]
% \SetAlgoLined
% \KwData{$\chi_{t-1}, u_t, z_t,m$}
% \textbf{monte\_carlo\_localization(}$\chi_{t-1}, u_t, z_t,m$\textbf{)}\\
% $\bar{\chi_t} = \chi_t = \emptyset$\;
% \For{$m$=1 to $M$ do}{
% $x^{[m]}_t$ = sample\_motion\_model( $u_t$ , $x^{[m]}_{t-1}$ )\;\label{alg:motion_model}
% $w^{[m]}__t$ = measurement\_model($z_t$, $x^{[m]}_t$ , $m$)\;\label{alg:measurement_model}
% $\bar{\chi} = \bar{\chi} + \langle x^{[m]}_t, w^{[m]}_t \rangle $\;}
% \For{$m$=1 to $M$ do \label{alg:resampling}} 
% {
% draw $i$ with probability $\alpha$  $w^{[i]}_t$\;
% add $x^{[i]}_t $ to $\chi_t$\;
% }
%  \KwResult{ $\chi_t$ }
...
\item \textbf{resample:} Redraw all particles, place new particles distributed according to the former computed weights, and reset all weights to one. (for loop starting in line : ~\ref{alg:resampling} ) %Such that at poses with high likelihood (weight) more particles will end up.
\end{enumerate}
...
% \SetAlgoLined
% \KwData{$\chi_{t-1}, u_t, z_t,m$}
% \textbf{augmented\_monte\_carlo\_localization(}$\chi_{t-1}, u_t, z_t,m$\textbf{)}\\
% static $w_{slow}$,$ w_{fast}$ \label{alg:augmented1}\;
% $\bar{\chi_t} = \chi_t = \emptyset$\;
% \For{$m$=1 to $M$ do}{
% $x^{[m]}_t$ = sample\_motion\_model( $u_t$ , $x^{[m]}_{t-1}$ )\;
% $w^{[m]}__t$ = measurement\_model($z_t$, $x^{[m]}_t$ , $m$)\;
% $\bar{\chi} = \bar{\chi} + \langle x^{[m]}_t, w^{[m]}_t \rangle $\;
% $w_{avg} = w_{avg} + \frac_{M}^{1} w^{[m]}_t $ \label{alg:augmented2}\;
% }
% $w_{slow} = w_{slow} + \alpha_{slow}(w_{avg} - w_{slow})$\label{alg:augmented3}\;
% $w_{fast} = w_{fast} + \alpha_{fast}(w_{avg} - w_{fast})$\label{alg:augmented4}\;
...
%  \KwResult{ $\chi_t$ }
% \caption{Augmented Monte Carlo Localization as depicted in \cite{ProbabilisticRobotsics}}
%\end{algorithm}
...
%  }
%  \KwResult{ $\bar{\chi_t}$ }
% \caption{Low variance resampling as described in \cite{ProbabilisticRobotsics}}
%\end{algorithm}
